...
class InstanceGenerator:
    ...
    In this class we generate the instances for each OPTW
    If given a seed it will always generate the same travel times
    """

    base_dir = path.dirname(path.dirname(path.dirname(path.abspath(__file__))))

    def __init__(self, n_instances, n_nodes, w=(80, 100),
                 ylim=(0, 50), xlim=(0, 200), prize='distance',
                 seed=123, data_dir='OPTW/instances', offset=0):
    ...
    def save_optw(self, i, inst, adj, x_dir='instances', adj_dir='adjs'):
        ...
        inst.to_csv(os.path.join(path_x, f'instance{self.offset + i:04}.csv'), index=False, sep=',')
        adj.to_csv(os.path.join(path_adj, f'adj-instance{self.offset + i:04}.csv'), index=False, sep=',')


if __name__ == '__main__':
    offset = 0
    for node in [50, 100, 200]:
        gen = InstanceGenerator(1, n_nodes=node, offset=offset)
