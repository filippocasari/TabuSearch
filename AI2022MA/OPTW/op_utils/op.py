...
def tour_check(tour, x, time_matrix, verbose=False):
    """
    ...
    for i in range(len(tour) - 1):
        node = int(tour[i])
        succ = int(tour[i + 1])
        time = time_matrix[node][succ]
        tour_time += time
        if tour_time > tw_high[succ]:
            feas = False
            rewards = 0
            return_to_depot = True
            if verbose:
                print("time window violated", tw_high[succ], "\ncurrent time ",
                      tour_time, "\nfrom node", node, " to ", succ)
            break
        elif tour_time < tw_low[succ]:
            tour_time = tw_low[succ]
            rewards += prizes[succ]
        else:
            rewards += prizes[succ]
    ...
    if not return_to_depot:
        raise Exception('A tour must reconnect back to the depot - node: 0')

    if tour_time > maxT:
        rewards = 0
        feas = False
        if verbose:
            print("total time violated", tour_time)
